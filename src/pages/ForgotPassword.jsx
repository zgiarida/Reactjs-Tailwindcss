...
export default function ForgotPassword() {
  ...
  const sendResetEmail = async (e) => {
    ...
    try {
      ...
      toast.success("Succes for reset password");
      navigate("/sign-in");
    } catch (error) {
      toast.error("cloud not send reset email password");
    }
  };
      ...
      <div className="flex justify-center items-center flex-wrap mb-12 px-6 py-12 max-w-6xl mx-auto ">
        ...
        <div className="w-full md:w-[67%] lg:w-[40%] lg:ml-8 sm:w-[78%]">
          <form onSubmit={sendResetEmail}>
            <input
              className="w-full py-2 px-4 text-xl text-gray-800 rounded-md border-gray-300 transition ease-in-out my-5"
              type="email"
              id="email"
              value={email}
              onChange={onChange}
            ...
            <div className="flex justify-between text-sm">
              ...
              <p>
                <Link
                  to="/sign-in"
                  className="text-blue-400 hover:text-blue-600 transition duration-200 ease-in-out italic"
                >
                  Sign In
                </Link>
              </p>
